...
/*This is a service that runs for all app life and check if there is any urgent Task and Show It in notification*/
public class NotificationService extends IntentService{

    private SQLiteDatabase db;
    boolean showOnlyOnce  //in charge to show the notification only once for each task
            , flag; //in charge to close the tread repeat
    /* constructor */
    ...
    protected void onHandleIntent(@Nullable Intent intent) {
        ...
        while(flag) {
            try {
                Thread.sleep(10000);  //thread that return every 10 sec
                ...
                {
                    cursor.moveToFirst();   //init all information from the first Task in the table
    ...
    public void setNotificationCall(String recvName,String clientName,String phoneNum,String address){
        ...
        Intent nevIntent = new Intent(Intent.ACTION_VIEW);  //Intent to go to google maps
...
//        SmsManager.getDefault().sendTextMessage(phoneNum,"","Hello " + recvName + ",\nIm the delivery boy.\nI'm on my way\nPlease be available on the next 2 hours.\nThank you.",null,null);
        Intent callIntent = new Intent(Intent.ACTION_CALL); //intent to call
        callIntent.setData(Uri.parse("tel:" + phoneNum));
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {    //ceck permission to call
